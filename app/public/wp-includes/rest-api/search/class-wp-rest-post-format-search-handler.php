...
class WP_REST_Post_Format_Search_Handler extends WP_REST_Search_Handler {
	...
	public function search_items( WP_REST_Request $request ) {
		 ...
		 */
		$args = apply_filters( 'rest_post_format_search_query', $args, $request );

		$found_ids = array();
		foreach ( $format_slugs as $index => $format_slug ) {
			if ( ! empty( $args['search'] ) ) {
				$format_string       = get_post_format_string( $format_slug );
				$format_slug_match   = stripos( $format_slug, $args['search'] ) !== false;
				$format_string_match = stripos( $format_string, $args['search'] ) !== false;
