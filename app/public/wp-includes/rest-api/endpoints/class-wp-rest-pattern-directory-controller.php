...
class WP_REST_Pattern_Directory_Controller extends WP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		$args       = array_intersect_key( $request->get_params(), $valid_query_args );

		$args['locale']             = get_user_locale();
		$args['wp-version']         = $wp_version; // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UndefinedVariable -- it's defined in `version.php` above.
		$args['pattern-categories'] = isset( $request['category'] ) ? $request['category'] : false;
		$args['pattern-keywords']   = isset( $request['keyword'] ) ? $request['keyword'] : false;

		$args = array_filter( $args );

		$transient_key = $this->get_transient_key( $args );
		...
		if ( ! $raw_patterns ) {
			$api_url = 'http://api.wordpress.org/patterns/1.0/?' . build_query( $args );
	 ...
	 * @since 6.0.0
	 *
	 * @param array $args Query arguments to generate a transient key from.
	...
	protected function get_transient_key( $args ) {
		...
		if ( isset( $args['slug'] ) ) {
			...
			$args['slug'] = wp_parse_list( $args['slug'] );
		...
		return 'wp_remote_block_patterns_' . md5( serialize( $args ) );
	}
}
