...
class WP_REST_Posts_Controller extends WP_REST_Controller {
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		foreach ( $prepared_args as $key => $value ) {
			 ...
			 */
			$args[ $key ] = apply_filters( "rest_query_var-{$key}", $value ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
		}

		if ( 'post' !== $this->post_type || ! isset( $args['ignore_sticky_posts'] ) ) {
			$args['ignore_sticky_posts'] = true;
		}

		// Map to proper WP_Query orderby param.
		if ( isset( $args['orderby'] ) && isset( $request['orderby'] ) ) {
			...
			if ( isset( $orderby_mappings[ $request['orderby'] ] ) ) {
				$args['orderby'] = $orderby_mappings[ $request['orderby'] ];
			}
		}
