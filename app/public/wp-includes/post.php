...
function get_pages( $args = array() ) {
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		...
		$parent   = -1;
		unset( $args['post__not_in'], $args['meta_key'], $args['meta_value'] );
		$hierarchical           = false;
		$args['post__in'] = wp_parse_id_list( $parsed_args['include'] );
	}
	...
	if ( ! empty( $parsed_args['authors'] ) ) {
		...
		if ( ! empty( $post_authors ) ) {
			...
			foreach ( $post_authors as $post_author ) {
				...
				$args['author__in'][] = (int) $post_author;
			}
		}
	}
	...
	if ( $orderby ) {
		$args['orderby'] = array_fill_keys( $orderby, $parsed_args['sort_order'] );
	}
	 ...
	 */
	$args = apply_filters( 'get_pages_query_args', $args, $parsed_args );
