...
function _admin_notice_post_locked() {
	...
	if ( $locked ) {
		...
		$preview_link = get_preview_post_link( $post->ID, $args );
...
function post_preview() {
	...
	if ( $is_autosave && $saved_post_id ) {
		$args['preview_id']    = $post->ID;
		$args['preview_nonce'] = wp_create_nonce( 'post_preview_' . $post->ID );
		...
		if ( isset( $_POST['_thumbnail_id'] ) ) {
			$args['_thumbnail_id'] = ( (int) $_POST['_thumbnail_id'] <= 0 ) ? '-1' : (int) $_POST['_thumbnail_id'];
		}
	}

	return get_preview_post_link( $post, $args );
}
