...
$_SERVER['REQUEST_URI'] = remove_query_arg( $args_to_remove, $_SERVER['REQUEST_URI'] );
		...
		case 'disable-auto-update-selected':
			...
			if ( 'enable-auto-update' === $action ) {
				...
				if ( 'enable-auto-update-selected' === $action ) {
					...
					$args       = array( 'enabled-auto-update-multi' => 'true' );
				} else {
					$new_auto_updates = array_diff( $auto_updates, $plugins );
					$args       = array( 'disabled-auto-update-multi' => 'true' );
				}
